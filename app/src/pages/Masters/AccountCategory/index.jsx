...
const AddEditAccountCategory = React.lazy(() => import("./add-edit-account-category"));
...
const TableHeaderFormat = (props) => {
  ...
  return [
    ...
    { field: 'cat_dtl', headerName: 'Category Detail', width: 150 },
    { field: 'cat_lvl', headerName: 'Same Level', width: 150 },
...
export default function AccountCategory() {
  ...
  const getAccountCategoryList = async () => {
    try {
      setLoading(true);
      const result = await axiosInstance.get(`/acccat/fetch`).then(res => res.data)
    ...
    <section className="inspection-entry-form">
        ...
        <Drawer anchor={"right"} open={open} onClose={handleClose} PaperProps={{ style: { width: 600 } }}> 
          <Suspense fallback={<Loader />}>
            <AddEditAccountCategory onClose={handleClose} /> 
          </Suspense>
        </Drawer>
        ...
        <Breadcrumbs aria-label="breadcrumb" style={{marginBottom: "1rem"}}>
          ...
          <Typography sx={{ color: 'text.primary' }}>Account Category</Typography>
        </Breadcrumbs>

        <Typography className="title" color="primary">
            <AccountTree color="primary" style={{ fontSize: "3rem", margin: "-10px 0" }} /> ACCOUNT CATEGORY
        </Typography>
