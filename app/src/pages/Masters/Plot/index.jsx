...
export default function Plot() {
  ...
  const getPlotList = async () => {
    try {
      setLoading(true);
      const result = await axiosInstance.get(`/plot/fetch`).then(res => res.data)
  ...
  return (
    <section className="inspection-entry-form">
        ...
        <Breadcrumbs aria-label="breadcrumb" style={{marginBottom: "1rem"}}>
          ...
          <Typography sx={{ color: 'text.primary' }}>Plot</Typography>
        </Breadcrumbs>

        <Typography className="title" color="primary">
            <GridOn color="primary" style={{ fontSize: "3rem", margin: "-10px 0" }} /> Plot
        </Typography>
