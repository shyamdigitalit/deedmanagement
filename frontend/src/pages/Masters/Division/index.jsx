...
const AddEditDivision = React.lazy(() => import("./add-edit-division"));
...
const TableHeaderFormat = (props) => {
  ...
  return [
    ...
    { field: 'div_code', headerName: 'Division Code', width: 150 },
    { field: 'div_desc', headerName: 'Division Desc', width: 150 },
...
export default function Division() {
  ...
  const getDivisionList = async () => {
    try {
      setLoading(true);
      const result = await axiosInstance.get(`/div/fetch`).then(res => res.data)
    ...
    <section className="inspection-entry-form">
        ...
        <Drawer anchor={"right"} open={open} onClose={() => handleClose()} PaperProps={{ style: { width: 600 } }}> 
          <Suspense fallback={<Loader />}>
            <AddEditDivision onClose={handleClose} /> 
          </Suspense>
        </Drawer>
        ...
        <Breadcrumbs aria-label="breadcrumb" style={{marginBottom: "1rem"}}>
          ...
          <Typography sx={{ color: 'text.primary' }}>Division</Typography>
        </Breadcrumbs>

        <Typography className="title" color="primary">
            <AccountTree color="primary" style={{ fontSize: "3rem", margin: "-10px 0" }} /> DIVISION
        </Typography>
        ...
        <Card>
            <CardContent style={{ padding: "0px" }}>
                ...
                <DataGrid sx={DataGridStyle} rows={divisionList} columns={TableHeaderFormat({currentPage, pageSize})} getRowId={row => row._id}
