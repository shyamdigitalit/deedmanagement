...
const AddEditDepartment = React.lazy(() => import("./add-edit-department"));
...
export default function Department() {
  ...
  const getDepartmentList = async () => {
    try {
      setLoading(true);
      const result = await axiosInstance.get(`/dept/fetch`).then(res => res.data)
    ...
    <section className="inspection-entry-form">
        ...
        <Drawer anchor={"right"} open={open} onClose={() => handleClose()} PaperProps={{ style: { width: 600 } }}> 
          <Suspense fallback={<Loader />}>
            <AddEditDepartment onClose={handleClose} /> 
          </Suspense>
        </Drawer>
        ...
        <Breadcrumbs aria-label="breadcrumb" style={{marginBottom: "1rem"}}>
          ...
          <Typography sx={{ color: 'text.primary' }}>Department</Typography>
        </Breadcrumbs>

        <Typography className="title" color="primary">
            <Badge color="primary" style={{ fontSize: "3rem", margin: "-10px 0" }} /> Department
        </Typography>
        ...
        <Card>
            <CardContent style={{ padding: "0px" }}>
                ...
                <DataGrid sx={DataGridStyle} rows={departmentList} columns={TableHeaderFormat({currentPage, pageSize})} getRowId={row => row._id}
