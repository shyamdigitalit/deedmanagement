...
const create = async (req, res) => {
    try {
        ...
        if (polcyDetails) {
            ...
            .map(elm => ({
                ...elm,
                installmentStatus: elm.installmentStatus !== '' ? elm.installmentStatus : 'Pending',
            }))
...
export const getAllPolicyDetails = async (filter) => {
    try {
        const status = filter?.status ? String(filter?.status).trim().toLowerCase() : '';
        const policyNo = filter?.policyNo ? String(filter?.policyNo).trim().toLowerCase() : '';
...
const read = async (req, res) => {
    try {
        ...
        const polcyopRecords = await getAllPolicyDetails({status})
